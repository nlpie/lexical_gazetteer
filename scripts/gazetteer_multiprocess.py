...
def get_gaz_matches(nlp, matcher, texts, span_left, span_right):
    ...
    for text in texts:
        ...
        for match_id, start, end in matches:
            ...
            if start - span_left >= 0:
                yield (string_id, doc[start:end].text, doc[(start - span_left):(end + span_right)].text)
            else:
                yield (string_id, doc[start:end].text, doc[start:(end + span_right)].text)
...
def core_process(nlp_lemma, nlp_neg, matcher, notes, doc_folder, 
                 span_left, span_right, dict_files_positive, dict_files_negative):
    
    for file in notes:
        with open(os.path.join(doc_folder, file), 'r') as f:
            for string_id, men, text in get_gaz_matches(nlp_neg, matcher, f, span_left, span_right):
...
def mention_using_gaz(nlp_lemma, gaz_csv_list, notes_for_training, doc_folder, dict_gaz, output):
    ...
    if ratio >= min_files:
        ...
        cores_needed = len(notes_for_training) / min_files
